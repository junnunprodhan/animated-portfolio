...
const EditBlog = ({ closeModal, openModal, blog }) => {
  const [isLoading, setIsLoading] = useState(false);
  const queryClient = useQueryClient();
  const { mutateAsync } = useMutation({
    mutationFn: async (payload) => {
      return await baseApiAxios.patch(`/blog/update/${blog?._id}`, payload);
    },
    onSuccess: () => {
  ...
  const onSubmit= async (data) => {
    try {
      ...
      const newBlog = {
        ...
        img: imageUpload ? imageUpload : blog?.img,
      };
